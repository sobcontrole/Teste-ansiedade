var PersonalityQuizzer = (function($, DOMBars, window, document){
	...
	var quizModel = Model({
		...
		calculateResult: function() {
			...
			if(topScore > 10 && topScore < 20){
				winnerId = 1;
			}
			if(topScore >=20 && topScore <= 30){
				winnerId = 2;
			}
			if(topScore > 30 && topScore <= 63){
				winnerId = 3;
			}
			...
			this.set("done", true)
			emailjs.send("default_service","template_NfPETx4x",{name: "Teste realizado" + winnerId.toString , notes: winnerId.toString});
